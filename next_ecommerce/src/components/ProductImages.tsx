...
const ProductImages = () => {
  ...
  return (
    <div className='flex flex-col p-4 md:flex-row w-full'>
        <div className='relative h-80 md:h-auto md:w-auto md:flex-1'>
            <Image src={images[clickedImage].url} fill alt='' className='rounded-md'/>
        </div>

        <div className='flex gap-3 m-2 md:flex-col md:px-5'>
            {images.map((image,i)=>(
                <div key={image.id} className='relative' onClick={()=>setClickedImage(i)}>
                    <Image src={image.url} alt='' height={80} width={80} className='rounded-md'/>
                </div>
            ))}
        </div>
    </div>
